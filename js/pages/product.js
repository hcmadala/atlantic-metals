...
async function initProductPage() {
    ...
    if (bc) {
        const safeMetal = escapeAttribute(p.metal);
        const metalLabel = escapeHTML(p.metal.charAt(0).toUpperCase() + p.metal.slice(1));
        bc.innerHTML = `
            <a href="index.html">Home</a>
            <span>›</span>
            <a href="products.html">All</a>
            <span>›</span>
            <a href="products.html?metal=${safeMetal}">${metalLabel}</a>
            <span>›</span>
...
document.getElementById("thumbScrollUp")?.addEventListener("click", () => scrollThumbs(-1));
document.getElementById("thumbScrollDown")?.addEventListener("click", () => scrollThumbs(1));
document.getElementById("detailQtyDown")?.addEventListener("click", () => changeDetailQty(-1));
document.getElementById("detailQtyUp")?.addEventListener("click", () => changeDetailQty(1));
document.getElementById("detailCartBtn")?.addEventListener("click", addToCartDetail);

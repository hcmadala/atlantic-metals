...
async function fetchAndUpdatePrices() {
  ...
  for (const code of ["XAU", "XAG", "XPT", "XPD"]) {
    ...
    if (pts.length < 2 && nyClose[code] != null && data[code] != null) {
      if (pts.length === 0 || pts[0] !== nyClose[code]) pts.unshift(nyClose[code]);
    }
    history[code] = pts;
  }
...
function updateMetal(name, code, data) {
  ...
  if (diff > 0) {
    if (arrowElement) { arrowElement.innerText = "▲"; arrowElement.className = "arrow positive"; }
    if (changeElement) changeElement.className = "metal-change positive";
  } else if (diff < 0) {
    if (arrowElement) { arrowElement.innerText = "▼"; arrowElement.className = "arrow negative"; }
    if (changeElement) changeElement.className = "metal-change negative";
  } else {
    if (arrowElement) { arrowElement.innerText = "•"; arrowElement.className = "arrow unchanged"; }
    if (changeElement) changeElement.className = "metal-change unchanged";
  }

  requestAnimationFrame(() => drawSparkline(code, canvas));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("resize", () => {
    for (const [name, code] of [
      ...
      drawSparkline(code, document.getElementById(name + "-chart"));
    }
  });
});